...
FIELDS = {
    ...
    # 유저에게 실제로 보이는 카테고리
    # (네 전시는 자주 바뀌고 이벤트 마케팅적인 목적에 의해 나뉘질 수 있기 때문에 그래도 mece한 분류가 관리라고 봐주시면 되어요)
    "display_category_ids": "전시(노출)카테고리ID(유저용)",
    ...
    # 운영자분들께서 운영하기 위한 카테고리
    # FROM dump.admin_categories 에 id에 해당
    # 주방 > 냄비/프라이팬/솥 > 프라이팬/그릴 > 일반프라이팬 -> convert to list
    # NOTE: 관리카테고리에 오류도 있음(예: 차렵이불로 분류되었으나 여름 이불인 경우)
    # NOTE: 예: 스테이크 나이프 -> https://ohou.se/productions/787594/selling, https://search-es2.stage.datahou.se/commerce_search/_doc/787594
    "admin_categories": "관리카테고리",
    # admin_categories였으나 일부 겸치는 카테고리 제거함
    "search_admin_categories": "관리카테고리(정제)",
    # 카테고리키워드 관련: https://wiki.dailyhou.se/pages/viewpage.action?pageId=126122228
