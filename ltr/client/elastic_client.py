...
class ElasticClient(BaseClient):
    ...
    def __init__(self, configs_dir='.'):
        ...
        self.elastic_ep = 'https://search-es2.stage.datahou.se/_ltr'
        self.es = Elasticsearch('https://search-es2.stage.datahou.se')
    ...
    def log_query(self, index, featureset, ids, params={}, return_body=False):
        params = {
            "query": {
                "bool": {
                    "filter": [
        ...
        for hit in resp['hits']['hits']:
            ...
            hit['_source']['ltr_features_name'] = []
            hit['_source']["_id"] = hit["_id"]  # add id

            for feature in hit['fields']['_ltrlog'][0]['ltr_features']:
                value = 0.0
                # e.g. feature = {'name': 'keyword_list.bm25_idfmaxnorm', 'value': 0.45435613}
                if 'value' in feature:
                    value = feature['value']

                hit['_source']['ltr_features'].append(value)
                hit['_source']['ltr_features_name'].append(feature["name"])
