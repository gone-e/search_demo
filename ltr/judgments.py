...
class Judgment:
    ...
    def toRanklibFormat(self):
        ...
        # NOTE: 지수표현을 읽지 못한다.
        featuresAsStrs = [f"{idx+1}:{feature:.8f}" for idx, feature in enumerate(self.features)]
...
# hyeok
def judgments_from_dataframe(df, diversity=0):
    # FIXME: 칼럼명이 하드코딩됨
    judgments = []
    for i, (query, gdf) in enumerate(df.groupby(["search_keyword"])):
        qid = i+1
        for _, row in gdf.iterrows():
            if len(gdf.grade.unique()) < diversity:
                continue
            judgments.append(Judgment(
                qid=qid,
                docId=row["docid"],
                grade=row["grade"],
                features=[v for k, v in dict(row).items() if k.startswith("f__")],
                keywords=query
            ))
    featuremap = [k.replace("f__", "") for k, v in dict(row).items() if k.startswith("f__")]
